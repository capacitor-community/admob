...
public class AdViewIdHelper {
  ...
  public static String getFinalAdId(
    AdOptions adOptions,
    AdRequest adRequest,
    String logTag,
    Context context
    ...
    if (adRequest.isTestDevice(context)) {
      Log.w(
        logTag,
        "This device is registered as Testing Device. The real Ad Id will be used"
      );
      return adOptions.adId;
    }
   ...
   * 2. Testing ID used: We request a testing ad with a not registered as a Testing device
   * 3. Real AdId used: We request a testing ad but this device is a Testing device
   */
  public static void assignIdToAdView(
